...
class _SpecData:
    ...
    def __str__(self, simple: bool = False) -> str:
        ...
        if not simple:
            s += (
                f" w/ {self.rejected_pixels.sum()}/{self.size} (rej.) {self.absorbed_pixels.sum()}/{self.size} (abs.) {self.valid_pixels.sum()}/{self.size} "
                f"(val.) {self.log_valid_pixels.sum()}/{self.size} (log-val.)"
            )
