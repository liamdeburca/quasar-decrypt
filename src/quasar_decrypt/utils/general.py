...
from numpy import argwhere, ndarray
...
def get_or_default(attr: str, kwargs: dict, default: dict):
...
def _fit_bounds(val, bounds):
    ...
    ub = bounds[1] if bounds is not None else inf

    return max([min([val, ub]), lb])


def val_or_default(default: dict, kwargs: dict, key: str):
    val = kwargs.get(key)
    out = default[key] if (val is None) else val
...
def nan_diffs(
    ...
    mask = isfinite(a) & isfinite(b) if mask is None else mask
...
def nan_residuals(
    ...
    mask = isfinite(y) & isfinite(f) & (dy > 0) if mask is None else mask
